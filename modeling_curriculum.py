...
class BERTAttention(nn.Module):
    def __init__(self, config, multi_params=None, houlsby=False, index=0):
        ...
        if config.our_attn and index >= config.num_hidden_layers - 8:
...
class BertModel(nn.Module):
    ...
    def __init__(self, config: BertConfig):
        ...
        self.scale_medium = nn.Embedding(config.num_tasks, 2)
        self.scale_hard = nn.Embedding(config.num_tasks, 3)
        self.pooler = BERTPooler(config)

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, i=0, diff_level = "easy"):
        ...
        if diff_level == "easy":
            sequence_output = all_encoder_layers[-3]
        if diff_level == "medium":
            sequence_output_1 = all_encoder_layers[-3]
            sequence_output_2 = all_encoder_layers[-2]
        ...
        if diff_level == "hard":
            sequence_output_1 = all_encoder_layers[-3]
            sequence_output_2 = all_encoder_layers[-2]
            sequence_output_3 = all_encoder_layers[-1]
            index = torch.tensor(i).to(embedding_output.device)
...
class BertForMultiTask(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, task_id, diff_level, name='cola', labels=None):
        _, pooled_output = self.bert(input_ids, token_type_ids, attention_mask, task_id, diff_level)
