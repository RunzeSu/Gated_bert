...
import tokenization
from modeling_curriculum import BertConfig, BertForSequenceClassification, BertForMultiTask
...
def main():
    ...
    if args.tasks == 'inf':
        ...
        diff_level =['medium', 'medium', 'medium']
        data_dirs = ['QNLI', 'MNLI', 'RTE']
    elif args.tasks == 'all':
        task_names =['cola', 'mrpc', 'mnli', 'rte', 'sts', 'sst', 'qqp', 'qnli']
        diff_level =['hard', 'medium', 'medium', 'medium', 'easy', 'medium', 'hard', 'medium']
        ...
        if args.multi:
            ...
            for n, p in model_dict.items():
                if 'aug' in n or 'mult' in n:
                    ...
                    if ('_list' in n) or ('task_emb' in n) or ('scale' in n):
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step in range(steps_per_epoch):
                ...
                loss, _ = model(input_ids, segment_ids, input_mask, task_id, diff_level[task_id], task_names[task_id], label_ids)
                if n_gpu > 1:
